...
## 1. 工作模式(单 AI)
- **你(接手本文件的 AI)**:直接写代码 + 构建 + commit + push,一步到位。涉及 UI 改动时**先描述方案让用户确认,确认后才写代码**(见 memory `feedback-ui-confirm-first`)。
- **用户**:只做"账号动作"(见 §3),其余 push 由 AI 经 SSH 完成,用户只需刷新网页验收。
- 高风险改动(数据结构变更/token 安全/大重构)多花一点时间自查;日常 UI/bug 直接改+push。
...
## 8. 每步工作流程
用户提需求 → AI 直接改代码(UI 改动先描述方案等确认)→ `npm run build` → `git add -A && git commit`(中文)`&& git push origin main`(SSH)→ 用户刷新线上验收。高风险改动多自查(数据格式/token/白屏守卫);日常直接改+push。
